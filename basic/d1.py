'''
    파이썬 <-> 데이터베이스
    파이썬으로 데이터베이스를 엑세스하여 쿼리를 전송, 수행 결과를 받아오는 방식
        - sql 수행
            - basic에서 수행
            - pymysql 패키지 사용
    ...
    5. 고객 테이블 생성
        ...
        CREATE TABLE `users` (
            ...
            `uid` VARCHAR(32) NOT NULL COMMENT '고객의 로그인 아이디' COLLATE 'utf8mb4_general_ci',
        ...
        CHANGE COLUMN `uid` `uid` VARCHAR(32) NOT NULL COMMENT '고객의 로그인 아이디' COLLATE 'utf8mb4_general_ci' AFTER `id`,
...
try:
    ...
    connection = my.connect(host             = 'localhost',            # 127.0.0.1, 서버 주소
                                # port        = 3306,                  # 포트 
                                user        = 'root',                  # 사용자 계정, root 계정 이 외 사용 권장 
